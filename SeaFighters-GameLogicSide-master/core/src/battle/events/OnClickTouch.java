...
public class OnClickTouch implements GestureListener {
	...
	private Seafighter seafighter;


	public OnClickTouch(int width, int height, Seafighter seafighter){
		GestureDetector gestureHandler = new GestureDetector(this);
		Gdx.input.setInputProcessor(gestureHandler);
		this.width=width;
		this.height=height;
		this.seafighter = seafighter;
	}
       ...
       // System.out.println("Coord X: "+worldX+" Y:"+worldY+" "+height+" "+width);
		...
		if (turn) {
			if (worldX <= 10 && worldX >= 0 && worldY >= start && worldY <= limit) {
				...
				if (this.screen == 2) {
					System.out.println("TAP: " + this.X + " " + this.Y);
					turn = false;
					this.seafighter.emitTurnChange();
				}
				return true;
			}
		}
